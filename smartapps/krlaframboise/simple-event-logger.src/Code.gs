...
var needToArchive = function(sheet, archiveOptions, data) {
	...
	switch (archiveOptions.type) {
		...
		case "Weekly":  // restart on Sunday
			//getDay()
			return (new Date(eventDate).getDay() != sheetLastDate.getDay() && new Date(eventDate).getDay() < sheetLastDate.getDay());  // Sunday - Saturday : 0 - 6	//&& (new Date(eventDate).getDay() == 0 || getDaysSince(eventDate, sheetFirstDate) > 7));		
